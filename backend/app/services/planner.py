...
def _build_refine_prompt(
    ...
    return (
        ...
        "A \"Property 'X' not valid in version N\" error means element X is not a "
        "valid child in that metadata type's schema — fix it by correcting the "
        "element to the schema-valid name (e.g. FlexiPage uses "
        "flexiPageRegions > itemInstances > componentInstance; there is NO "
        "componentInstances element), NOT by changing the apiVersion.\n"
        "A \"Template ... doesn't exist\" or \"... doesn't exist\" error means you "
        "referenced a FlexiPage template or component name that Salesforce does "
        "not provide — replace it with a real one (for a RecordPage use "
        "<template><name>flexipage:recordHomeTemplateDesktop</name></template>). "
        "Never use slds* names as a template or componentName; they are CSS "
        "classes, not metadata.\n\n"
